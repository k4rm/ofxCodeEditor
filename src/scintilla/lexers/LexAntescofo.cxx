 ...
 * @file LexANTESCOFO.cxx
 * Lexer for ANTESCOFO
 *
 * Improved by Mike Lischke <mike.lischke@oracle.com>
 * Adopted from LexSQL.cxx by Anders Karlsson <anders@ANTESCOFO.com>
        ...
        if (keywordlists[3]->InList(s))
          ...
          if (keywordlists[5]->InList(s))
            ...
            if (keywordlists[6]->InList(s))
              sc.ChangeState(SCE_ANTESCOFO_USER1 | activeState);
            else
              if (keywordlists[7]->InList(s))
                sc.ChangeState(SCE_ANTESCOFO_USER2 | activeState);
              else
                if (keywordlists[8]->InList(s))
                  sc.ChangeState(SCE_ANTESCOFO_USER3 | activeState);
  delete [] s;
}
...
{
  if (activeState == 0)
    sc.SetState(SCE_ANTESCOFO_DEFAULT);
  else
    sc.SetState(SCE_ANTESCOFO_HIDDENCOMMAND);
}

static void ForwardDefaultState(StyleContext& sc, int activeState)
{
  if (activeState == 0)
    sc.ForwardSetState(SCE_ANTESCOFO_DEFAULT);
  else
    sc.ForwardSetState(SCE_ANTESCOFO_HIDDENCOMMAND);
}

static void ColouriseAntescofoDoc(unsigned int startPos, int length, int initStyle, WordList *keywordlists[],
                            Accessor &styler)
{
	StyleContext sc(startPos, length, initStyle, styler, 127);
  int activeState = (initStyle == SCE_ANTESCOFO_HIDDENCOMMAND) ? HIDDENCOMMAND_STATE : initStyle & HIDDENCOMMAND_STATE;
	...
	for (; sc.More(); sc.Forward())
  {
    ...
    {
      case SCE_ANTESCOFO_OPERATOR:
        SetDefaultState(sc, activeState);
        break;
      case SCE_ANTESCOFO_NUMBER:
        // We stop the number definition on non-numerical non-dot non-eE non-sign char.
        if (!IsANumberChar(sc.ch))
          SetDefaultState(sc, activeState);
        break;
      case SCE_ANTESCOFO_IDENTIFIER:
        ...
        if (!IsAWordChar(sc.ch))
        {
          ...
          if (MASKACTIVE(sc.state) == SCE_ANTESCOFO_FUNCTION && sc.ch != '(')
          {
            if (activeState > 0)
              sc.ChangeState(SCE_ANTESCOFO_HIDDENCOMMAND);
            else
              sc.ChangeState(SCE_ANTESCOFO_DEFAULT);
          }

          SetDefaultState(sc, activeState);
        }
        break;
      case SCE_ANTESCOFO_VARIABLE:
        if (!IsAWordChar(sc.ch))
          SetDefaultState(sc, activeState);
        break;
      case SCE_ANTESCOFO_SYSTEMVARIABLE:
        if (!IsAWordChar(sc.ch))
        {
          ...
          if (keywordlists[4]->InList(&s[2]))
            sc.ChangeState(SCE_ANTESCOFO_KNOWNSYSTEMVARIABLE | activeState);
          delete [] s;

          SetDefaultState(sc, activeState);
        }
        break;
      case SCE_ANTESCOFO_QUOTEDIDENTIFIER:
        if (sc.ch == '`')
        {
          if (sc.chNext == '`')
      ...
      case SCE_ANTESCOFO_COMMENT:
        ...
        break;
      case SCE_ANTESCOFO_COMMENTLINE:
        if (sc.atLineStart)
          SetDefaultState(sc, activeState);
        break;
      case SCE_ANTESCOFO_SQSTRING:
        if (sc.ch == '\\')
      ...
      case SCE_ANTESCOFO_DQSTRING:
        ...
        break;
      case SCE_ANTESCOFO_PLACEHOLDER:
        if (sc.Match('}', '>'))
        {
          sc.Forward();
          ForwardDefaultState(sc, activeState);
    ...
    if (sc.state == SCE_ANTESCOFO_HIDDENCOMMAND && sc.Match('*', '/'))
    {
      activeState = 0;
      sc.Forward();
      ForwardDefaultState(sc, activeState);
    ...
    if (sc.state == SCE_ANTESCOFO_DEFAULT || sc.state == SCE_ANTESCOFO_HIDDENCOMMAND)
    {
      switch (sc.ch)
      {
        case '$':
					sc.Forward(1); // Skip past @.
					if (IsAWordStart(sc.ch))
					{
						sc.SetState(SCE_ANTESCOFO_VARIABLE | activeState);
						sc.Forward(1); // Skip past @.
					}
					else
						sc.SetState(SCE_ANTESCOFO_OPERATOR | activeState);
					break;
        case '`':
          sc.SetState(SCE_ANTESCOFO_QUOTEDIDENTIFIER | activeState);
          break;
        case ';':
          sc.SetState(SCE_ANTESCOFO_COMMENTLINE | activeState);
          break;
        case '\'':
          sc.SetState(SCE_ANTESCOFO_SQSTRING | activeState);
          break;
        case '\"':
          sc.SetState(SCE_ANTESCOFO_DQSTRING | activeState);
          break;
        default:
          if (IsADigit(sc.ch) || (sc.ch == '.' && IsADigit(sc.chNext)))
            sc.SetState(SCE_ANTESCOFO_NUMBER | activeState);
          else
            if (IsAWordStart(sc.ch))
              sc.SetState(SCE_ANTESCOFO_IDENTIFIER | activeState);
            else
              if (sc.Match('/', '*'))
              {
                sc.SetState(SCE_ANTESCOFO_COMMENT | activeState);
              ...
              {
                sc.SetState(SCE_ANTESCOFO_PLACEHOLDER | activeState);
              }
              else
                if (sc.Match(";") || sc.Match("//"))
                {
                  // Special ANTESCOFO single line comment.
                  sc.SetState(SCE_ANTESCOFO_COMMENTLINE | activeState);
									if (sc.Match("//")) sc.Forward(2);
									if (sc.Match(";")) sc.Forward(1);

                  // Check the third character too. It must be a space or EOL.
                  if (sc.ch != '\n' && sc.ch != '\r')
                    sc.ChangeState(SCE_ANTESCOFO_OPERATOR | activeState);
                }
                else
  ...
  if (sc.state == SCE_ANTESCOFO_IDENTIFIER)
  {
    CheckForKeyword(sc, keywordlists, activeState);

    // Additional check for function keywords needed.
    // A function name must be followed by an opening parenthesis.
    if (sc.state == SCE_ANTESCOFO_FUNCTION && sc.ch != '(')
      SetDefaultState(sc, activeState);
  }
...
{
	return MASKACTIVE(style) == SCE_ANTESCOFO_COMMENT;
}

//--------------------------------------------------------------------------------------------------

extern bool MatchIgnoreCase(Accessor &styler, int currentPos, const char *s);
/**
...
static void FoldAntescofoDoc(unsigned int startPos, int length, int initStyle, WordList *[], Accessor &styler)
{
	bool foldComment = styler.GetPropertyInt("fold.comment") != 0;
	bool foldCompact = styler.GetPropertyInt("fold.compact", 1) != 0;
	bool foldOnlyBegin = styler.GetPropertyInt("fold.antescofo.only.begin", 0) != 0;
	...
	int style = initStyle;
  int activeState = (style == SCE_ANTESCOFO_HIDDENCOMMAND) ? HIDDENCOMMAND_STATE : style & HIDDENCOMMAND_STATE;
  ...
  for (unsigned int i = startPos; length > 0; i++, length--)
  {
    ...
    activeState = (style == SCE_ANTESCOFO_HIDDENCOMMAND) ? HIDDENCOMMAND_STATE : style & HIDDENCOMMAND_STATE;
    ...
    switch (MASKACTIVE(style))
    {
      ...
      case SCE_ANTESCOFO_COMMENTLINE:
        break;
      case SCE_ANTESCOFO_HIDDENCOMMAND:
        /*
        if (endPending)
        {
      ...
      case SCE_ANTESCOFO_OPERATOR:
        ...
        break;
      case SCE_ANTESCOFO_MAJORKEYWORD:
      case SCE_ANTESCOFO_KEYWORD:
      case SCE_ANTESCOFO_FUNCTION:
      case SCE_ANTESCOFO_PROCEDUREKEYWORD:
...
static const char * const AntescofoWordListDesc[] = {
	"Major Keywords",
	"Keywords",
	"Database Objects",
	"Functions",
...
LexerModule lmAntescofo(SCLEX_ANTESCOFO, ColouriseAntescofoDoc, "Antescofo", FoldAntescofoDoc, AntescofoWordListDesc, 7);

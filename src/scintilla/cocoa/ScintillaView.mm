...
NSString *SCIMarginClickNotification = @"SCIMarginClick";
NSString *SCIAutoCompleteNotification = @"SCIAutoCompleteNotification";
...
{
	NSLog(@"ScintillaView dragginEntered");
...
/**
 * Sends a new notification of the given type and user info to the default notification center.
 */
- (void) sendNotification: (NSString*) notificationName withUserInfo: (NSDictionary*) userInfo
{
  NSNotificationCenter* center = [NSNotificationCenter defaultCenter];
  [center postNotificationName: notificationName object: self userInfo: userInfo];
}
    ...
    case WM_NOTIFY:
    {
      ...
      switch (scn->nmhdr.code)
      {
        ...
        case SCN_MARGINCLICK:
        {
          ...
          // Notification to handle special events in editors
          // e.g. like setting breakpoints or do other actions when the gutter is clicked.
          NSMutableDictionary* info = [NSMutableDictionary dictionary];
          [info setValue: [NSNumber numberWithInt: line] forKey: @"line"];
          [info setValue: [NSNumber numberWithInt: scn->modifiers] forKey: @"modifiers"];
          [info setValue: [NSNumber numberWithInt: scn->margin] forKey: @"margin"];
          [editor sendNotification: SCIMarginClickNotification withUserInfo: info];
          break;
        };
        case SCN_MODIFIED:
        {
          ...
          {
            // Notification used to ease parsing of text changes (e.g. for marking errors).
            long line = [editor getGeneralProperty: SCI_LINEFROMPOSITION parameter: scn->position];
            NSMutableDictionary* info = [NSMutableDictionary dictionary];
            [info setValue: [NSNumber numberWithInt: line] forKey: @"line"];
            [info setValue: [NSNumber numberWithInt: scn->linesAdded] forKey: @"linesAdded"];

            [editor sendNotification: NSTextDidChangeNotification withUserInfo: info];
          }
          break;
        }
        case SCN_ZOOM:
...
{
  // HTML hex code of the form #rgb or #rrggbb.
  ...
  {
    // A color name. Scan all available color catalogs for that.
    // Unfortunately the color comparison in those lists is case-sensitive, which forces us
    // to do an extra (time costly) roundtrip.
    NSArray* colorLists = [NSColorList availableColorLists];
    for (int i = 0; i < [colorLists count]; i++)
    {
      NSArray* keys = [[colorLists objectAtIndex: i] allKeys];
      for (int j = 0; j < [keys count]; j++)
      {
        if ([[keys objectAtIndex: j] caseInsensitiveCompare: fromHTML] == NSOrderedSame)
        {
          [self setColorProperty: property
                       parameter: parameter
                           value: [[colorLists objectAtIndex: i] colorWithKey: [keys objectAtIndex: j]]];
          return;
        }
      }
    }

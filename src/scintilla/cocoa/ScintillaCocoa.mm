...
{
  //wMain= [view retain];// Don't retain since we're owned by view, which would cause a cycle
  wMain = view;
...
ScintillaCocoa::~ScintillaCocoa()
{
  SetTicking(false);
  [timerTarget release];
  //container is no longer retained as usual pattern is that container owns content, not the opposite
